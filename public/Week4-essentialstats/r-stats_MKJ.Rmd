---
title: "Essential Statistics in R"
date: "March 4, 2019"
...
Packages needed today are readr, dplyr, ggplot2. We can load them all by loading the tidyverse package
...
Download the data files called **nhanes.csv** and a data description file (filename **nhanes_dd.csv**) that lists and describes each variable in our NHANES dataset.
...
# Now compute mean age for each gender of each race
nh %>% 
...
```{r, results="hide"}
summary(nh)
#check out the missings
```

### Missing data

Let's try taking the mean of a different variable, either the dplyr way or the simpler `$` way. I'll use base r syntax.
...
mean(nha$Pulse, na.rm=TRUE)
# nha$Pulse %>% mean(na.rm=TRUE) #dplyr way
```
...
```{r, include=FALSE, eval=FALSE}
range(nha$BPDia, na.rm=TRUE)
#hmm, someone has BP=0...may need to look more closely at this person to see if there are other errors
```

3. What's the variance and standard deviation for income among the adult participants?
```{r, include=FALSE, eval=FALSE}
var(nha$Income, na.rm=TRUE)
sd(nha$Income, na.rm=TRUE)
```

4. Create a histogram of TotChol (Total Cholesterol).
```{r, include=FALSE, eval=FALSE}
ggplot(nha, aes(TotChol)) + geom_histogram()
```

## Statistics orientation

Take a good long look at the flowchart for basic statisitical tests. We will mention most of these today. This is new, so if you have any ideas for improvement, please let me know

![flowchart](HypothesisTestingFlowChart.png)
...
Let's start with EDA for this question. I particularly like density plots colored by group. This type of plot helps you see the shape of the distribution of the variable to assess equal variance. Also helps you see if there is a noticeable difference in groups.
...
```{r normality}
#histogram
ggplot(nha, aes(x = Height)) + geom_histogram() + facet_wrap(~Gender)
...
1. First, create a plot to explore the distribution of the variables and the relationship between these two variables.
```{r, include=FALSE, eval=FALSE}
...
# (multiply times 2 because this is a two-tailed test)
...
There is so much more to learn with regression models. Next week, we'll spend the whole lesson on regressions.
