...
original <- read_csv("brauer2007_messy.csv")
...
write_csv(original, "brauer2007_halfclean.csv")
...
```{r}
nutrient_lookup <- data_frame(nutrient = c("G", "L", "N", "P", "S", "U"), nutrient_name = c("Glucose", "Leucine", "Ammonia", "Phosphate", "Sulfate", "Uracil"))
...
```{r, echo=FALSE}
clean %>% filter(symbol=="ADH2" & rate==0.05) %>% select(nutrient, expression)
```
