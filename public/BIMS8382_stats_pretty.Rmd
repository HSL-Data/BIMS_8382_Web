...
We will use the `read_csv()` function from the tidyverse to get our data into R. Once we've loaded it, we can type the name of the object itself (`brauer`) to see it printed to the screen.
...
Now, let's see what are the `unique` values of Status using base R and then using a dplyr pipeline
...
# whoa, definitely not normally distributed, but I know that these data are typically log transformed, so let's try that
